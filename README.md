...
Regarding our dataset, we have already discussed some simplifying assumptions we have made in the "CPT Calculations from Previous Model" subsection of the "Training" section. However, one other potential bias in our dataset is that the Kaggle post does not make it entirely clear what the source of the data is, only that the "cards are distributed as seen in a casino". So, there could be some potential issues on how reliable the values are to human players.